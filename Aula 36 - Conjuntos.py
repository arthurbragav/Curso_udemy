...
Conjuntos
 ...
 Dito isso, na mesma forma que na matemática:
  - Sets (Conjuntos), não possuem valores duplicados;
  - Sets (Conjuntos), não possuem valores ordenados;
  - Elementos não são acessados via índice, ou seja, Conjuntos não são indexados;

Conjuntos são bons para se utilizar quando precisamos armazenar elementos
mas não nos importamos com a ordenação deles. Quando não precisamos nos
...
s = set({1, 2, 3, 4, 5, 5, 6, 7, 2, 3}) # Repare que tem valores repetidos

print(s)
print(type(s))

# Obs: Ao criar um conjunto, caso seja adicionado um valor já existente, o mesmo será
# ignorado sem gerar erros, e não fará parte do conjunto;
...
# O que você faria? Um loop na lista?
...
# Imagine que temos 2 conjuntos: um contendo estudantes do curso Python e o outro com estudantes de Java
...
unicos1 = estudantes_python.union(estudantes_java) #  Tanto faz a ordem, só vai mudar a ordem
print(unicos1)
...
so_python = estudantes_python.difference(estudantes_java) # Nesse caso a ordem importa
print(so_python)

so_java = estudantes_java.difference(estudantes_python) # Nesse caso a ordem importa
print(so_java)
